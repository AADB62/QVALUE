...
statsAnalysis <- function(HRVIndices, easyOptions) {
  ...
  pVals$adj.p.value <- p.adjust_expanded(pVals$p.value, method = easyOptions$method)
  ...
  if (numberGroups > 2) {
    ...
    if (!is.null(pairwiseComparisons)) {
      pairwiseComparisons$adj.p.value <- p.adjust_expanded(pairwiseComparisons$p.value, method = easyOptions$method)
